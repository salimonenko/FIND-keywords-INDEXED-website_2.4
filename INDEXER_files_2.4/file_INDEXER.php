...
if(isset($_REQUEST['DO_working'])){ // 1. Запуск индексирования ФАЙЛОВ сайта (исходя из перечня их имен в файле files.txt)
    if(isset($_GET['DO_working']) && $_GET['DO_working'] === 'true'){ // Вызывается при запуске событий сервера
        header('Content-Type: text/event-stream');
        header('Cache-Control: no-cache');

        if(!isset($_GET['last_managed_string']) || !preg_match('/^\d{1,10}$/', $_GET['last_managed_string'])){
            $mess = '<p class="error_mes">Неверное значение номера строки: допустимо только положительное целое число, не более 10 цифр.</p>';
...
// Имя файла перекодированное (только для вывода в виде информации на экран). Актуально, когда в имени файла содержится, например, кириллица
        ...
        if(!is_readable($file_name_ABS)){ // Если файл вдруг не существует (мало ли...) или заблокирован в данный момент
            ...
            $mess = '<p class="info_mes">'. ($i+1).'. Индексируется файл: <b>'.$file_name_encoded. '... </b></p>';
            sendMsg(time(), $mess, false);
        }
...
// 7.10. Если дошли до сюда (в каждой итерации цикла перебора индексируемых файлов), то выводим подтверждение об успехе. Также сообщаем номер строки в файле files.txt, содержащей имя только что проиндексированного файла и его индекс
        $mess = '<p class="info_mes">OK</p>'. '<script>show_index('. ($i+1). ',"'. str_replace("\\", "\\\\", $file_name_encoded) .'",'. $total_size. ')</script>'. '<br/>'; // +1, т.к. номерация строк в файле (в Notepad++) начинается с 1, а не с 0
        sendMsg(time(), $mess, false);
